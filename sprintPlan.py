...
title.text = 'Sprint {} plan'.format(sprint_id - sprint_id_deviation)
...
for assignee in assignee_names:
  ...
  p = tf.add_paragraph()
  p.font.size = Pt(22)
  p.text = "Planned for sprint {}".format(sprint_id - sprint_id_deviation)
  p.level = 0

  # Query Jira for all tickets assigned to this assignee in the current sprint
  tickets = jira_client.search_issues('sprint in ({}) and assignee = {}'.format(sprint_id, assignee))
  ...
  # Query Jira for all tickets assigned to this assignee in the current sprint
  tickets = jira_client.search_issues('sprint in ({}) and assignee = {} and status in (Open, "Open (Assigned To)", Unconfirmed, "In Progress")'.format(sprint_id - 1, assignee))
  ...
  p = tf.add_paragraph()
  p.font.size = Pt(22)
  p.text = "Continue from sprint {}".format(sprint_id - sprint_id_deviation - 1)
  p.level = 0
...
presentation.save('sprint {} plan.pptx'.format(sprint_id - sprint_id_deviation))
